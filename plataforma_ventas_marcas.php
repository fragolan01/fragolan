...
if ($v7=="despliega") {
	...
	$query_plataforma_ventas_marcas = "SELECT * FROM plataforma_ventas_marcas WHERE id_dominio='".$id_dominio."' ORDER BY nombre";
	...
	while ($line_plataforma_ventas_marcas = mysql_fetch_assoc($result_plataforma_ventas_marcas)) {
        ...
        $data_plataforma_ventas_marcas0=$line_plataforma_ventas_marcas["id_dominio"];
        $data_plataforma_ventas_marcas0=$line_plataforma_ventas_marcas["id_proveedor"];
        $data_plataforma_ventas_marcas0=$line_plataforma_ventas_marcas["id_catego"];
		$data_plataforma_ventas_marcas3=$line_plataforma_ventas_marcas["nombre"];

		echo "<nobr>".$data_plataforma_ventas_marcas0.".- (<a href='index.php?v7=actualizar&v13=".$data_plataforma_ventas_marcas0."'>Actualizar</a> | <a href='index.php?v7=eliminar&v13=".$data_plataforma_ventas_marcas0."'>Eliminar</a>)marca: Nombre: ".$data_plataforma_ventas_marcas3."</nobr>";
		echo "<br>";
	}
}
if ($v7=="actualizar") {
	...
	if ($v13) {
		...
		$query_plataforma_ventas_marcas = "SELECT id,id_dominio,nombre FROM plataforma_ventas_marcas WHERE id='".$v13."' AND id_dominio='".$id_dominio."'";
		$result_plataforma_ventas_marcas = mysql_query($query_plataforma_ventas_marcas) or die('Query failed: plataforma_ventas_marcas ' . mysql_error());
		while ($line_plataforma_ventas_marcas = mysql_fetch_assoc($result_plataforma_ventas_marcas)) {
			$data_plataforma_ventas_marcas0=$line_plataforma_ventas_marcas["id"];
			$data_plataforma_ventas_marcas3=$line_plataforma_ventas_marcas["nombre"];
		...
		echo "<form name='actualiza_plataforma_ventas_marcas' method='post' action='index.php?v7=actualizalo&v13=".$v13."'>";
			echo "<nobr>Nombre de la Marca: <input type='text' name='nombre' value='".$data_plataforma_ventas_marcas3."' size='50'><input type='submit' name='actualizar' value='ACTUALIZAR &raquo;'></nobr>";
		echo "</form>";
	}
	else {
		echo "<br><br>";
		...
		echo "<form name='inserta_plataforma_ventas_marcas' method='post' action='index.php?v7=actualizalo'>";
			echo "<nobr>Nombre de la Marca: <input type='text' name='nombre' value='' size='50'><input type='submit' name='insertar' value='INSERTAR &raquo;'></nobr>";
		echo "</form>";
	}
}
if ($v7=="actualizalo") {
	if ($v13) {
		...
		if (!$nombre) {
			...
			$sql_plataforma_ventas_marcas = "UPDATE plataforma_ventas_marcas SET nombre='".$nombre."' WHERE id='".$v13."' AND id_dominio='".$id_dominio."'";
