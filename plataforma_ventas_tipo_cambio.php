...
if ($v7=="despliega") {
	...
	while ($line_plataforma_ventas_tipo_cambio = mysql_fetch_assoc($result_plataforma_ventas_tipo_cambio)) {
		...
		$data_plataforma_ventas_tipo_cambio10=$line_plataforma_ventas_tipo_cambio["un_mes"];
		...
		echo 
			"<nobr>".
				...
				$data_plataforma_ventas_tipo_cambio9. " Un mes: ".
				$data_plataforma_ventas_tipo_cambio10.
			"</nobr>";
		echo "<br>";
	}
}
...
if ($v7=="actualizar") {
	...
	if ($v13) {
		...
		$query_plataforma_ventas_tipo_cambio = "SELECT id,id_dominio,fecha,normal,preferencial,un_dia,una_semana,dos_semanas,tres_semanas,un_mes FROM plataforma_ventas_tipo_cambio WHERE id='".$v13."' AND id_dominio='".$id_dominio."'";
		$result_plataforma_ventas_tipo_cambio = mysql_query($query_plataforma_ventas_tipo_cambio) or die('Query failed: plataforma_ventas_tipo_cambio ' . mysql_error());
		while ($line_plataforma_ventas_tipo_cambio = mysql_fetch_assoc($result_plataforma_ventas_tipo_cambio)) {
			$data_plataforma_ventas_tipo_cambio0=$line_plataforma_ventas_tipo_cambio["id"];
			$data_plataforma_ventas_tipo_cambio3=$line_plataforma_ventas_tipo_cambio["fecha"];
		...
		echo "<form name='actualiza_plataforma_ventas_tipo_cambio' method='post' action='index.php?v7=actualizalo&v13=".$v13."'>";
			echo "<nobr>
					...
					Tres semanas: <input type='text' name='tres_semanas' value='".$data_plataforma_ventas_tipo_cambio9."' size='50'>
					Un mes: <input type='text' name='un_mes' value='".$data_plataforma_ventas_tipo_cambio10."' size='50'>
		...
		echo "<form name='inserta_plataforma_ventas_tipo_cambio' method='post' action='index.php?v7=actualizalo'>";
			echo "<nobr>
					...
					Un mes: <input type='text' name='un_mes' value='' size='50'>
...
if ($v7=="actualizalo") {
	if ($v13) {
		...
		if (!$normal) {
			...
			$sql_plataforma_ventas_tipo_cambio = "UPDATE plataforma_ventas_tipo_cambio SET fecha='".$fecha."', normal='".$normal."', preferencial='".$preferencial."', un_dia='".$un_dia."', una_semana='".$una_semana."', dos_semanas='".$dos_semanas."', tres_semanas='".$tres_semanas."', un_mes='".$un_mes."' WHERE id='".$v13."' AND id_dominio='".$id_dominio."'";
