	...
	while ($line_plataforma_ventas_envios = mysql_fetch_assoc($result_plataforma_ventas_envios)) {
		...
		echo 
			"<nobr>".
				...
				$data_plataforma_ventas_envios3.
			"</nobr>";
...
if ($v7=="actualizar") {
	...
	if ($v13) {
		...
		echo "<form name='inserta_plataforma_ventas_envios' method='post' action='index.php?v7=actualizalo'>";
			echo "<nobr>Envio: <input type='text' name='envio' value='' size='50'><input type='submit' name='insertar' value='INSERTAR &raquo;'></nobr>";
		echo "</form>";
	}
}
if ($v7=="actualizalo") {
	if ($v13) {
		...
		if (!$envio) {
			...
			echo "<font color=green>PERFECTO! Envioi insertado...</font>";
